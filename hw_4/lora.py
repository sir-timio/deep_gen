...
class LoraLoaderMixin:
    ...
    def _best_guess_weight_name(
        ...
        targeted_files = list(
            filter(
                lambda x: all(substring not in x for substring in unallowed_substrings),
                targeted_files,
            )
        )
                ...
                if isinstance(component, nn.Module) and hasattr(component, "_hf_hook"):
                    ...
                    if not is_sequential_cpu_offload:
                        is_sequential_cpu_offload = isinstance(
                            component._hf_hook, AlignDevicesHook
                        )
    ...
    def save_lora_weights(
        ...
        if not (
            unet_lora_layers or text_encoder_lora_layers or transformer_lora_layers
        ):
            raise ValueError(
        ...
        if text_encoder_lora_layers:
            state_dict.update(
                pack_weights(text_encoder_lora_layers, cls.text_encoder_name)
            )
    ...
    def fuse_lora(
        ...
        if fuse_unet:
            ...
            unet.fuse_lora(
                lora_scale, safe_fusing=safe_fusing, adapter_names=adapter_names
            )
    ...
    def set_adapters(
        ...
        invert_list_adapters = {
            adapter: [
                part for part, adapters in list_adapters.items() if adapter in adapters
            ]
        ...
        for adapter_name, weights in zip(adapter_names, adapter_weights):
            if isinstance(weights, dict):
                ...
                if text_encoder_2_lora_weight is not None and not hasattr(
                    self, "text_encoder_2"
                ):
                    logger.warning(
                ...
                for part_weight, part_name in zip(
                    ...
                    if (
                        part_weight is not None
                        and part_name not in invert_list_adapters[adapter_name]
                    ):
                        logger.warning(
        ...
        if hasattr(self, "text_encoder"):
            self.set_adapters_for_text_encoder(
                adapter_names, self.text_encoder, text_encoder_lora_weights
            )
        if hasattr(self, "text_encoder_2"):
            self.set_adapters_for_text_encoder(
                adapter_names, self.text_encoder_2, text_encoder_2_lora_weights
            )
    ...
    def get_list_adapters(self) -> Dict[str, List[str]]:
        ...
        if hasattr(self, "text_encoder_2") and hasattr(
            self.text_encoder_2, "peft_config"
        ):
            set_adapters["text_encoder_2"] = list(
                self.text_encoder_2.peft_config.keys()
            )
